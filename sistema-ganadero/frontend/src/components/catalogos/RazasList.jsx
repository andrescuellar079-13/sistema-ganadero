...
export default function RazasList() {
  ...
  const [orientacionFilter, setOrientacionFilter] = useState('todos')
  ...
  const razasFiltradas = useMemo(() => {
    ...
    if (orientacionFilter !== 'todos') {
      filtered = filtered.filter(r => r.orientacion === orientacionFilter)
    }
    ...
    return filtered
  }, [razas, searchTerm, orientacionFilter, estadoFilter])

  const limpiarFiltros = () => {
    setSearchTerm('')
    setOrientacionFilter('todos')
  ...
  const hayFiltrosActivos = searchTerm || orientacionFilter !== 'todos' || estadoFilter !== 'todos'
      ...
      <div className="mb-4 bg-white rounded-lg shadow p-4">
        ...
        {showFilters && (
          <div className="mt-4 pt-4 border-t grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            >
              <option value="todos">Todas las orientaciones</option>
        ...
        {hayFiltrosActivos && (
          <div className="mt-3 text-sm text-gray-500">
            {razasFiltradas.length} de {razas.length} razas encontradas
          </div>
        )}
      </div>
      ...
      {razasFiltradas.length === 0 ? (
        <div className="text-center py-20 bg-gray-50 rounded-lg">
          <p className="text-gray-500">
            {hayFiltrosActivos ? 'No hay razas que coincidan con la búsqueda' : 'No hay razas registradas'}
          </p>
        </div>
      ) : (
